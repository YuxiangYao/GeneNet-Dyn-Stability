/*
    ...
    version: 1.0
*/
#ifndef ANALYSIS_BIONETS_WINDOWS_H_INCLUDED
#define ANALYSIS_BIONETS_WINDOWS_H_INCLUDED
#include <io.h>
//#include <dirent.h>
#include <stdio.h>
#include <stdlib.h>
...
double BioNets::AverageSensitivity(){
    ...
    for(ii=0;ii<nn;ii++){
        if(in_deg[ii]>1){// No-input nodes are ignored. 
            ...
            sum+=rrr;}}
    return sum/effective;// [NOTE] The effective maybe zero, so can return NaN.
}
void BioNets::BoolFunFractionalAnalysis(FILE *fp){
    ...
    for(ii=0;ii<nn;ii++){
        if(in_deg[ii]>1){// No-input nodes are ignored.
            ...
            // IF cana?
            tmp_c.maptab=Maps[ii];tmp_if=tmp_c.is_Cana();RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);printf("C->");
            // IF dominate?
            tmp_d.Initial();tmp_d.In_Enumerate();tmp_d.Domin_Pre_Major();
            tmp_d.maptab=Maps[ii];tmp_if=tmp_d.is_Domin();RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);printf("D->");
            tmp_d.Reset();
            // IF effective?
            tmp_e.maptab=Maps[ii];tmp_if=tmp_e.is_Effec();fprintf(fp,"%d,",tmp_if);printf("E->");
            // IF post-2?
            tmp_p.maptab=Maps[ii];tmp_if=tmp_p.is_Post();RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);printf("P->");
            // IF monotanic? (Pure, Mixed)
            tmp_u.maptab=Maps[ii];tmp_if=tmp_u.is_Unate();RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);
            tmp_if=tmp_u.is_Mixed_Unate();RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);printf("M->");
            // IF threshold? (Standard, ) // St pn_1,theta=0,
            tmp_t.maptab=Maps[ii];tmp_if=tmp_t.is_Thres();RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);printf("T->");
            //tmp_if=Z3_Logic_Threshold(Maps[ii],tmp_t.k,tmp_t.length,0,NULL);// 0/1
            //RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);printf("T1->");
            //tmp_if=Z3_Logic_Threshold(Maps[ii],tmp_t.k,tmp_t.length,1,NULL);// +/-
            //RBF+=tmp_if;fprintf(fp,"%d,",tmp_if);printf("T2->");
            // Random.
            fprintf(fp,"%d\n",0==RBF);}}
}
...
void BioNets_Logical::GetBioNetwork(char *Dir){
    ...
    char external_comp[128];
    std::string FilePath;
    std::vector<std::string> FileList;
    struct _finddata_t FileInfo;// Structor of file information.
    File_Handle=_findfirst(FilePath.assign(Dir).append("//*").c_str(),&FileInfo);
    if(File_Handle!=-1){
        do{
            if(strcmp(FileInfo.name, ".")!=0&&strcmp(FileInfo.name, "..")!=0){
                if(strcmp(FileInfo.name,"external_components.ALL.txt")!=0){
                    FileList.push_back(FilePath.assign(FileInfo.name));
                    SysSize++;}
                else {// External components filename.
                    strcpy(external_comp,
                        FilePath.assign(Dir).append("//").append(FileInfo.name).c_str());}}
...
class BioNets_Threshold:public BioNets{
    ...
    private:
        void SetNewNode(int **LinkType,int *InDeg,int target,int source,int type);// Set new topology.
        void AttachNode(int **LinkType,int *InDeg,int target,int source,int type);// Add at existed topology.
};
...
void BioNets_Threshold::SetNewNode(int **LinkType,int *InDeg,int target,int source,int type){
    InDeg[target]++;
    LinkType[target]=(int*)malloc(sizeof(int));// Update link type.
    LinkType[target][0]=type;
    Parents[target]=(int*)malloc(sizeof(int));// Update the parents.
    Parents[target][0]=source;
}
void BioNets_Threshold::AttachNode(int **LinkType,int *InDeg,int target,int source,int type){
    ...
    // Update parents.
    ipt=(int*)malloc(length+INT_BIT);
    memcpy(ipt,Parents[target],length);
    ipt[degree]=source;
    free(Parents[target]);
    Parents[target]=ipt;
}
        ...
        while(!feof(fp)){
            ...
            if(kk>0){// Valid read.
                ...
                if(ind[child]>0){// The indeg has been updated.
                    AttachNode(topo_link,ind,child,parent,sum);}
                else {
                    SetNewNode(topo_link,ind,child,parent,sum);}}}
...
#endif // ANALYSIS_BIONETS_WINDOWS_H_INCLUDED
